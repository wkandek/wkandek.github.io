...
Claude just added a  a telegram and discord interface for claude code. I installed it and I am testing it out now. This is an experimental stage at Anthropic, so expect some rough edges...

Installation for Telegram are here: `https://code.claude.com/docs/en/channels`

Follow the instructions on Anthropic's site. Basically you will need to create a telegram account for you. I had one already so I can skip that step. Then a bot needs to be created - this is what Claude will need access to communicate with you. Create the bot via DM to @botfather. It will step you through the process, basically selecting a name for it. the URL "https://<name>_bot" will be used later to access it for the first time in a DM. You will be issued a token that you need to copy and supply to claude later.

On the claude code side, there is (undocuemnted) dependency. I had to install bun and add bun into the path for my terminal. On my mac this was done via: `curl -fsSL https://bun.sh/install | bash` and it ended up in ~/.bun/bin, so `export PATH=$PATH:~/.bun/bin` (add to the .profile for your shell on my mac) took care of the access to bun. Then a plugin needs to be installed in Claude via `/plugin install telegram@claude-plugins-official`, configured via `/telegram:configure 82....92:AAFBpE....aLcXCYyLxthc`. The token comes from the bot creation procss above. Then claude needs to be restarted with `claude --channels plugin:telegram@claude-plugins-official`.
